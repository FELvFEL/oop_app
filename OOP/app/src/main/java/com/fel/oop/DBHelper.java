...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public DBHelper(@Nullable Context context) {
        super(context, "criminals.db", null, 5);
    }

    @Override
    public void onCreate(SQLiteDatabase db) {
        db.execSQL("CREATE TABLE " + CRIMINALS + " (" + COLUMN_NAME + " TEXT, " + COLUMN_BIRTHDATE + " TEXT, " + COLUMN_COUNTRY + " TEXT, " + COLUMN_CRIME + " TEXT, " + COLUMN_SIGNS + " TEXT, " + COLUMN_URI + " TEXT);");
    }
    ...
    public LinkedList<Criminal> GetAll() {
        ...
        if (cursor.moveToFirst()) {
           do{
               ...
               Criminal criminal = new Criminal(cursor.getString(id_n),cursor.getString(id_y),cursor.getString(id_country),cursor.getString(id_crime),cursor.getString(id_signs),cursor.getString(id_u));
               list.add(criminal);
